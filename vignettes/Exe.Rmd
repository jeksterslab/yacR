...
```{r}
yac_symbol <- Exe(expr1, R = FALSE, format = "ysym")
str(yac_symbol)
```
...
```{r}
string <- Exe(expr1, R = FALSE, format = "str")
str(string)
```
...
```{r}
latex <- Exe(expr1, R = FALSE, format = "tex")
str(latex)
```

LaTeX mathematics can be displayed using the following.

Inline using inline operator `r` enclosed by `$`.
$`r latex`$

Centered equation using the inline operator `r` enclosed by `$$`.
$$`r latex`$$
...
```{r}
yac_symbol <- Exe(expr2, R = FALSE, format = "ysym")
str(yac_symbol)
```
...
```{r}
string <- Exe(expr2, R = FALSE, format = "str")
str(string)
```
...
```{r}
latex <- Exe(expr2, R = FALSE, format = "tex")
str(latex)
```
